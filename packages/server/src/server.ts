...
function parseAgentLabSchedule(value: unknown): AgentLabSchedule {
  ...
  if (kind === "interval") {
    const seconds = clampInt(schedule.seconds, {
      min: 60,
      max: 7 * 24 * 60 * 60,
      name: "schedule.seconds",
    });
    return { kind: "interval", seconds };
  }
  if (kind === "daily") {
    const hour = clampInt(schedule.hour, {
      min: 0,
      max: 23,
      name: "schedule.hour",
    });
    const minute = clampInt(schedule.minute, {
      min: 0,
      max: 59,
  ...
  if (kind === "weekly") {
    const weekday = clampInt(schedule.weekday, {
      min: 1,
      max: 7,
  ...
  throw new ApiError(
    400,
    "invalid_payload",
    "schedule.kind must be interval, daily, or weekly",
  );
}
...
function validateAgentLabAutomationId(value: unknown): string {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(raw)) {
    throw new ApiError(
      400,
      "invalid_payload",
      "automation id must match /^[a-zA-Z0-9_-]+$/",
    );
  }
  return raw;
}

async function readAgentLabAutomations(
  workspaceRoot: string,
): Promise<AgentLabAutomationStore> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      normalized.push({
        ...
        lastRunSessionId:
          typeof record.lastRunSessionId === "string"
    ...
    throw new ApiError(
      422,
      "invalid_json",
      "Failed to parse Agent Lab automations",
    );
  }
}

async function writeAgentLabAutomations(
  workspaceRoot: string,
  store: AgentLabAutomationStore,
): Promise<void> {
  const path = resolveAgentLabAutomationsPath(workspaceRoot);
...
function createRoutes(
  ...
  addRoute(routes, "PATCH", "/workspace/:id/config", "client", async (ctx) => {
    ...
    if (!opencode && !openwork) {
      throw new ApiError(
        400,
        "invalid_payload",
        "opencode or openwork updates required",
      );
    }
    ...
    await requireApproval(ctx, {
      ...
      paths: [
        ...
        openwork ? openworkConfigPath(workspace.path) : null,
  ...
  addRoute(
    ...
    async (ctx) => {
      ...
      throw new ApiError(
        410,
        "engine_reload_deprecated",
        "OpenWork-managed engine reload is disabled",
        {
          workspaceId: workspace.id,
  ...
  addRoute(
    ...
    async (ctx) => {
      ...
      const baseUpdatedAt =
        typeof baseUpdatedAtRaw === "number" &&
        Number.isFinite(baseUpdatedAtRaw)
          ? baseUpdatedAtRaw
          : null;
        ...
        ? {
            ...
            repo:
              typeof repoPayload.repo === "string"
                ...
                : undefined,
            ref:
              typeof repoPayload.ref === "string" ? repoPayload.ref : undefined,
          }
        : undefined;
  ...
  addRoute(
    ...
    async (ctx) => {
      const workspace = await resolveWorkspace(config, ctx.params.id);
      const result = await getInstructions(workspace.path);
      return jsonResponse(result);
    },
  );
  ...
  addRoute(
    ...
    async (ctx) => {
      ...
      await requireApproval(ctx, {
        workspaceId: workspace.id,
        action: "instructions.update",
        summary: "Update workspace system instructions",
        paths: [join(workspace.path, ".opencode", "instructions.md")],
      });
      const result = await saveInstructions(workspace.path, content);
      await ensureInstructionsInConfig(workspace.path);
      await recordAudit(workspace.path, {
        id: shortId(),
        workspaceId: workspace.id,
        actor: ctx.actor ?? { type: "remote" },
        action: "instructions.update",
        target: result.path,
        summary: "Updated system instructions",
        timestamp: Date.now(),
      });
      emitReloadEvent(ctx.reloadEvents, workspace, "config", {
        type: "config" as const,
        action: result.action === "created" ? "added" : "updated",
        path: result.path,
      });
      return jsonResponse(result);
    },
  ...
  addRoute(
    ...
    async (ctx) => {
      ...
      const id = body.id
        ? validateAgentLabAutomationId(body.id)
        : `agentlab_${shortId().replace(/-/g, "")}`;
      ...
      return jsonResponse(
        { items: next.items, updatedAt: next.updatedAt },
        201,
      );
    },
  );
  ...
  addRoute(
    routes,
    "DELETE",
    "/workspace/:id/agentlab/automations/:automationId",
    "client",
    async (ctx) => {
      ensureWritable(config);
      requireClientScope(ctx, "collaborator");
      const workspace = await resolveWorkspace(config, ctx.params.id);
      const automationId = validateAgentLabAutomationId(
        ctx.params.automationId,
      );
  ...
  addRoute(
    routes,
    "POST",
    "/workspace/:id/agentlab/automations/:automationId/run",
    "client",
    async (ctx) => {
      requireClientScope(ctx, "collaborator");
      const workspace = await resolveWorkspace(config, ctx.params.id);
  ...
  addRoute(
    routes,
    "GET",
    "/workspace/:id/agentlab/automations/logs",
    "client",
    async (ctx) => {
      const workspace = await resolveWorkspace(config, ctx.params.id);
  ...
  addRoute(
    routes,
    "GET",
    "/workspace/:id/agentlab/automations/logs/:automationId",
    "client",
    async (ctx) => {
      const workspace = await resolveWorkspace(config, ctx.params.id);
      const automationId = validateAgentLabAutomationId(
  ...
  addRoute(
    routes,
    "DELETE",
    "/workspace/:id/scheduler/jobs/:name",
    "client",
    async (ctx) => {
      ensureWritable(config);
      requireClientScope(ctx, "collaborator");
      const workspace = await resolveWorkspace(config, ctx.params.id);
      const name = ctx.params.name ?? "";
      const { job, jobFile, systemPaths } = await resolveScheduledJob(
        name,
        workspace.path,
      );
      await requireApproval(ctx, {
    ...
    async (ctx) => {
      ...
      const limit =
        Number.isFinite(parsedLimit) && parsedLimit > 0
          ...
          : 20;
      const { items, total, path } = await listSoulHeartbeats(
        workspace.path,
        limit,
      );
...
function parseSoulHeartbeatLine(
  rawLine: string,
  lineIndex: number,
): SoulHeartbeatEntry | null {
  ...
  const nextAction =
    typeof nextActionRaw === "string" && nextActionRaw.trim()
      ? nextActionRaw.trim()
      : null;
  const summaryRaw =
  ...
  const summary =
    ...
    nextAction ||
    (looseEnds.length
      ? `Loose ends: ${looseEnds.slice(0, 2).join("; ")}`
      : "(no summary)");
...
function configIncludesSoulInstruction(
  ...
  if (Array.isArray(instructions)) {
    return instructions.some(
      (entry) => typeof entry === "string" && entry.includes(target),
    );
  }
  return false;
}
...
async function getSoulStatus(workspaceRoot: string): Promise<SoulStatus> {
  ...
  try {
    ...
    heartbeatCommandExists = commands.some(
      (command) => command.name === "soul-heartbeat",
    );
  } catch {
  ...
  const estimatedIntervalMs = heartbeatJob
    ? estimateCronIntervalMs(heartbeatJob.schedule)
    : null;
  const staleAfterMs = enabled
    ? Math.max(
        estimatedIntervalMs ? estimatedIntervalMs * 2 : 24 * 60 * 60_000,
        30 * 60_000,
      )
    : null;

  const parsedLastHeartbeat = lastHeartbeatAt
  ...
  if (!enabled) {
    ...
    (heartbeatJob?.lastRunStatus ?? "") === "failed" ||
    Boolean(heartbeatJob?.lastRunError?.trim())
  ) {
    state = "error";
  } else if (overdue) {
    state = "stale";
