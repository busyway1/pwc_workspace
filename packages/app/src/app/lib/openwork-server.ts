...
function normalizeOpenworkBundleInviteIntent(
  value: string | null | undefined,
): OpenworkBundleInviteIntent {
...
export function buildOpenworkConnectInviteUrl(input: {
  ...
  const workspaceUrl =
    normalizeOpenworkServerUrl(input.workspaceUrl ?? "") ?? "";
  if (!workspaceUrl) return "";

  const base =
    normalizeOpenworkServerUrl(input.appUrl ?? "") ??
    DEFAULT_OPENWORK_CONNECT_APP_URL;
...
export function readOpenworkConnectInviteFromSearch(
...
export function readOpenworkBundleInviteFromSearch(
  input: string | URLSearchParams,
) {
  const search =
    typeof input === "string"
  ...
  const intent = normalizeOpenworkBundleInviteIntent(
    search.get(OPENWORK_INVITE_PARAM_BUNDLE_INTENT),
  );
...
export function writeOpenworkServerSettings(
...
export function hydrateOpenworkServerSettingsFromEnv() {
  ...
  const envUrl =
    typeof import.meta.env?.VITE_OPENWORK_URL === "string"
      ...
      : "";
  const envPort =
    typeof import.meta.env?.VITE_OPENWORK_PORT === "string"
...
export function createOpenworkServerClient(options: {
  ...
  return {
    ...
    status: () =>
      requestJson<OpenworkServerDiagnostics>(baseUrl, "/status", {
        token,
        hostToken,
        timeoutMs: timeouts.status,
      }),
    capabilities: () =>
      requestJson<OpenworkServerCapabilities>(baseUrl, "/capabilities", {
        token,
        hostToken,
    ...
    opencodeRouterBindings: (filters?: {
      channel?: string;
      identityId?: string;
    }) => {
      const search = new URLSearchParams();
      if (filters?.channel?.trim())
        search.set("channel", filters.channel.trim());
      if (filters?.identityId?.trim())
        search.set("identityId", filters.identityId.trim());
      const suffix = search.toString();
      const path = suffix
        ? `/opencode-router/bindings?${suffix}`
        : "/opencode-router/bindings";
      return requestJsonRaw<OpenworkOpenCodeRouterBindingsResult>(
        baseUrl,
        path,
        { token, hostToken, timeoutMs: timeouts.opencodeRouter },
      );
    ...
    opencodeRouterTelegramIdentities: () =>
      requestJsonRaw<OpenworkOpenCodeRouterTelegramIdentitiesResult>(
        baseUrl,
        "/opencode-router/identities/telegram",
        { token, hostToken, timeoutMs: timeouts.opencodeRouter },
      ),
    opencodeRouterSlackIdentities: () =>
      requestJsonRaw<OpenworkOpenCodeRouterSlackIdentitiesResult>(
        baseUrl,
        "/opencode-router/identities/slack",
        { token, hostToken, timeoutMs: timeouts.opencodeRouter },
      ),
    listWorkspaces: () =>
      requestJson<OpenworkWorkspaceList>(baseUrl, "/workspaces", {
        token,
        hostToken,
    ...
    exportWorkspace: (workspaceId: string) =>
      requestJson<OpenworkWorkspaceExport>(
        baseUrl,
    ...
    getOpenCodeRouterTelegramIdentities: (
      ...
      const query =
        typeof options?.healthPort === "number"
          ? `?healthPort=${encodeURIComponent(String(options.healthPort))}`
          : "";
    ...
    getOpenCodeRouterBindings: (
      ...
      if (filters?.identityId?.trim())
        search.set("identityId", filters.identityId.trim());
      if (typeof filters?.healthPort === "number")
        search.set("healthPort", String(filters.healthPort));
      const suffix = search.toString();
    ...
    sendOpenCodeRouterMessage: (
      ...
      return requestJson<OpenworkOpenCodeRouterSendResult>(
        baseUrl,
        primaryPath,
        {
          token,
    ...
    getInstructions: (workspaceId: string) =>
      ...
      }>(baseUrl, `/workspace/${workspaceId}/instructions`, {
        token,
        hostToken,
      }),
    saveInstructions: (workspaceId: string, content: string) =>
      requestJson<{ path: string; action: "created" | "updated" }>(
        baseUrl,
        `/workspace/${workspaceId}/instructions`,
        { token, hostToken, method: "PUT", body: { content } },
      ),
    listScheduledJobs: (workspaceId: string) =>
      requestJson<{ items: ScheduledJob[] }>(
        baseUrl,
        `/workspace/${workspaceId}/scheduler/jobs`,
        { token, hostToken },
      ),
    deleteScheduledJob: (workspaceId: string, name: string) =>
      requestJson<{ job: ScheduledJob }>(
        baseUrl,
        `/workspace/${workspaceId}/scheduler/jobs/${encodeURIComponent(name)}`,
        {
          token,
          hostToken,
          method: "DELETE",
    ...
    getSoulStatus: (workspaceId: string) =>
      requestJson<OpenworkSoulStatus>(
        baseUrl,
        `/workspace/${encodeURIComponent(workspaceId)}/soul/status`,
        {
          token,
          hostToken,
        },
    ...
    listSoulHeartbeats: (workspaceId: string, limit = 20) =>
      requestJson<{
        items: OpenworkSoulHeartbeatEntry[];
        total: number;
        path: string;
      }>(
        baseUrl,
    ...
    readFileBatch: (sessionId: string, paths: string[]) =>
      requestJson<OpenworkFileReadBatchResult>(
        baseUrl,
    ...
    writeFileBatch: (
      ...
      requestJson<OpenworkFileWriteBatchResult>(
        baseUrl,
    ...
    runFileBatchOps: (
      ...
      requestJson<OpenworkFileOpsBatchResult>(
        baseUrl,
    ...
    listArtifacts: (workspaceId: string) =>
      requestJson<OpenworkArtifactList>(
        baseUrl,
...
export type OpenworkServerClient = ReturnType<
  typeof createOpenworkServerClient
>;
