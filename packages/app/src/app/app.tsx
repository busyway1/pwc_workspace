...
function normalizeSharedBundleImportIntent(
  value: string | null | undefined,
): SharedBundleImportIntent {
...
function parseSharedBundle(value: unknown): SharedBundleV1 {
  ...
  if (type === "skills-set") {
    const skills = Array.isArray(record.skills)
      ? record.skills
          .map(readSkillItem)
          .filter((item): item is SharedSkillItem => Boolean(item))
      : [];
    if (!skills.length) {
...
function buildImportPayloadFromBundle(bundle: SharedBundleV1): {
  ...
  if (Array.isArray(workspace.skills) && workspace.skills.length)
    payload.skills = workspace.skills;
  if (Array.isArray(workspace.commands) && workspace.commands.length)
    payload.commands = workspace.commands;

  const importedSkillsCount = Array.isArray(workspace.skills)
    ? workspace.skills.length
    : 0;
  return { payload, importedSkillsCount };
}

function parseSharedBundleDeepLink(
  rawUrl: string,
): SharedBundleDeepLink | null {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
...
function parseRemoteConnectDeepLink(
  rawUrl: string,
...
export default function App() {
  ...
  const [openworkServerSettings, setOpenworkServerSettings] =
    createSignal<OpenworkServerSettings>({});
  const [openworkServerUrl, setOpenworkServerUrl] = createSignal("");
  const [openworkServerStatus, setOpenworkServerStatus] =
    createSignal<OpenworkServerStatus>("disconnected");
  const [openworkServerCapabilities, setOpenworkServerCapabilities] =
    createSignal<OpenworkServerCapabilities | null>(null);
  const [openworkServerCheckedAt, setOpenworkServerCheckedAt] = createSignal<
    number | null
  >(null);
  const [openworkServerWorkspaceId, setOpenworkServerWorkspaceId] =
    createSignal<string | null>(null);
  const [openworkServerHostInfo, setOpenworkServerHostInfo] =
    createSignal<OpenworkServerInfo | null>(null);
  ...
  const [openworkAuditEntries, setOpenworkAuditEntries] = createSignal<
    OpenworkAuditEntry[]
  >([]);
  const [openworkAuditStatus, setOpenworkAuditStatus] = createSignal<
    "idle" | "loading" | "error"
  >("idle");
  const [openworkAuditError, setOpenworkAuditError] = createSignal<
    string | null
  >(null);
  ...
  createEffect(() => {
    ...
    const bundleInvite = readOpenworkBundleInviteFromSearch(
    ...
    if (!invite) {
      ...
      if (
        invite.startup === "server" &&
        untrack(onboardingStep) === "welcome"
      ) {
    ...
    const cleanedConnect = stripOpenworkConnectInviteFromUrl(
  ...
  createEffect(() => {
    ...
    let fontZoom = applyAndPersistFontZoom(
      readStoredFontZoom(window.localStorage) ?? 1,
    );
  ...
  const checkOpenworkServer = async (
    ...
    try {
      ...
      if (
        ...
        return {
          status: "limited" as OpenworkServerStatus,
          capabilities: null,
        };
      }
      return {
        status: "disconnected" as OpenworkServerStatus,
        capabilities: null,
      };
    }
    ...
    try {
      ...
      return {
        status: "connected" as OpenworkServerStatus,
        capabilities: caps,
      };
    } catch (error) {
      if (
  ...
  const [lastKnownConfigSnapshot, setLastKnownConfigSnapshot] =
    createSignal("");
  ...
  const [sessionModelById, setSessionModelById] = createSignal<
    Record<string, ModelRef>
  >({});
  const [sessionModelOverridesReady, setSessionModelOverridesReady] =
    createSignal(false);
  ...
  const [providerAuthError, setProviderAuthError] = createSignal<string | null>(
    null,
  );
  const [providerAuthMethods, setProviderAuthMethods] = createSignal<
  ...
  const describeProviderError = (error: unknown, fallback: string) => {
    ...
    if (status && !heading.includes(String(status)))
      lines.push(`Status: ${status}`);
    if (provider && !heading.includes(provider))
  ...
  async function sendPrompt(draft?: ComposerDraft) {
    ...
    const resolvedDraft: ComposerDraft = draft ?? {
      mode: "prompt",
      parts: fallbackText
        ? [{ type: "text", text: fallbackText } as ComposerPart]
        : [],
  ...
  async function renameSessionTitle(sessionID: string, title: string) {
    ...
    await refreshSidebarWorkspaceSessions(
  ...
  async function deleteSessionById(sessionID: string) {
    ...
    const params = root
      ? { sessionID: trimmed, directory: root }
      : { sessionID: trimmed };
  ...
  > {
    ...
    const list = await listCommandsTyped(
      c,
      workspaceStore.activeWorkspaceRoot().trim() || undefined,
    );
  ...
  async function startProviderAuth(
    ...
    try {
      ...
      if (oauthIndex === -1) {
        throw new Error(
          `No OAuth flow available for ${resolved}. Use an API key instead.`,
        );
      }

      const auth = unwrap(
        await c.provider.oauth.authorize({
          providerID: resolved,
          method: oauthIndex,
        }),
      );
  ...
  const [scheduledJobsStatus, setScheduledJobsStatus] = createSignal<
    string | null
  >(null);
  const [scheduledJobsBusy, setScheduledJobsBusy] = createSignal(false);
  const [scheduledJobsUpdatedAt, setScheduledJobsUpdatedAt] = createSignal<
    number | null
  >(null);
  const [soulStatusByWorkspaceId, setSoulStatusByWorkspaceId] = createSignal<
    Record<string, OpenworkSoulStatus | null>
  >({});
  const [activeSoulHeartbeats, setActiveSoulHeartbeats] = createSignal<
  ...
  // Instructions state
  const [instructionsContent, setInstructionsContent] = createSignal("");
  const [instructionsLastModified, setInstructionsLastModified] = createSignal<
    string | null
  >(null);

  // Commands state
  const [commands, setCommands] = createSignal<OpenworkCommandItem[]>([]);
  const [commandsStatus, setCommandsStatus] = createSignal<string | null>(null);

  // MCP OAuth modal state
  const [mcpAuthModalOpen, setMcpAuthModalOpen] = createSignal(false);
  const [mcpAuthEntry, setMcpAuthEntry] = createSignal<
    (typeof MCP_QUICK_CONNECT)[number] | null
  >(null);
  ...
  const providerDefaults = createMemo(
    () => globalSync.data.provider.default ?? {},
  );
  ...
  const serializeSessionModelOverrides = (
  ...
  const formatConfigWithDefaultModel = (
    content: string | null,
    model: ModelRef,
  ) => {
  ...
  const formatModelVariantLabel = (value: string | null) => {
    ...
    return (
      MODEL_VARIANT_OPTIONS.find((option) => option.value === normalized)
        ?.label ?? "None"
    );
  };
  ...
  const [sidebarSessionsByWorkspaceId, setSidebarSessionsByWorkspaceId] =
    createSignal<Record<string, SidebarSessionItem[]>>({});
  const [
    sidebarSessionStatusByWorkspaceId,
    setSidebarSessionStatusByWorkspaceId,
  ] = createSignal<Record<string, SidebarWorkspaceSessionsStatus>>({});
  const [
    sidebarSessionErrorByWorkspaceId,
    setSidebarSessionErrorByWorkspaceId,
  ] = createSignal<Record<string, string | null>>({});
  ...
  const resolveSidebarClientConfig = (workspaceId: string) => {
    const workspace =
      workspaceStore.workspaces().find((entry) => entry.id === workspaceId) ??
      null;
    ...
    if (workspace.workspaceType === "local") {
      ...
      const auth: OpencodeAuth | undefined =
    ...
    if (workspace.remoteType === "openwork") {
      ...
      const auth: OpencodeAuth | undefined = token
        ? { token, mode: "openwork" }
        : undefined;
      return {
        baseUrl,
  ...
  const refreshSidebarWorkspaceSessions = async (workspaceId: string) => {
    ...
    sidebarRefreshSeqByWorkspaceId[id] =
      (sidebarRefreshSeqByWorkspaceId[id] ?? 0) + 1;
    ...
    try {
      ...
      if (!directory) {
        try {
          ...
          const discovered = normalizeDirectoryQueryPath(
            pathInfo.directory ?? "",
          );
          if (discovered) {
      ...
      const queryDirectory =
        normalizeDirectoryQueryPath(directory) || undefined;
  ...
  const refreshAllSidebarWorkspaceSessions = async (
    prioritizeWorkspaceId?: string | null,
  ) => {
    ...
    const ordered = prioritize
      ? [
          ...list.filter((ws) => ws.id === prioritize),
          ...list.filter((ws) => ws.id !== prioritize),
        ]
      : list;
  ...
  const refreshLocalSidebarWorkspaceSessions = async (
    prioritizeWorkspaceId?: string | null,
  ) => {
  ...
  createEffect(() => {
      ...
      .map((ws) => {
        const root =
          ws.workspaceType === "local"
            ...
            : (ws.directory?.trim() ?? "");
        const base =
          ws.workspaceType === "local" ? "" : (ws.baseUrl?.trim() ?? "");
        const remoteType =
          ws.workspaceType === "remote" ? (ws.remoteType ?? "") : "";
        const token =
          ws.remoteType === "openwork" ? (ws.openworkToken?.trim() ?? "") : "";
    ...
    if (
      engineKey === lastSidebarEngineKey &&
      workspaceKey === lastSidebarWorkspaceKey
    )
      return;
    ...
    pruneSidebarSessionState(
      new Set(workspaceStore.workspaces().map((ws) => ws.id)),
    );
    ...
    if (engineChanged && !workspacesChanged) {
      void refreshLocalSidebarWorkspaceSessions(
        workspaceStore.activeWorkspaceId(),
      ).catch(() => undefined);
      return;
    }

    void refreshAllSidebarWorkspaceSessions(
      workspaceStore.activeWorkspaceId(),
    ).catch(() => undefined);
  });
  ...
  createEffect(() => {
    ...
    const wsId = (
      workspaceStore.connectingWorkspaceId() ??
      workspaceStore.activeWorkspaceId()
    ).trim();
    if (!wsId) return;
    ...
    if (status === "ready") {
      const activeWorkspace =
        workspaceStore
          .workspaces()
          .find((workspace) => workspace.id === wsId) ?? null;
      ...
      const scopedSessions = activeWorkspaceRoot
        ? allSessions.filter(
            (session) =>
              normalizeDirectoryPath(session.directory) === activeWorkspaceRoot,
          )
        : allSessions;
  ...
  const sidebarWorkspaceGroups = createMemo<WorkspaceSessionGroup[]>(() => {
    ...
    const connectingWorkspaceId =
      workspaceStore.connectingWorkspaceId()?.trim() ?? "";
    ...
    for (const workspace of workspaces) {
      ...
      const workspaceIdKey =
        ...
        "";
      const directoryKey = normalizeDirectoryPath(
        workspace.directory?.trim() ?? workspace.path?.trim() ?? "",
      );
      const identityKey = workspaceIdKey
        ? `id:${workspaceIdKey}`
        : directoryKey
          ? `dir:${directoryKey}`
          : "";
      ...
      const existingIsPriority =
        ...
        existingWorkspace.id === connectingWorkspaceId;
      const currentIsPriority =
        workspace.id === activeWorkspaceId ||
        workspace.id === connectingWorkspaceId;
      ...
      const resolveWorkspace = async () => {
        try {
          ...
          const directoryHint = normalizeDirectoryPath(
            active.directory?.trim() ?? active.path?.trim() ?? "",
          );
  ...
  const resolveSharedBundleWorkerTarget = () => {
    ...
    const localHostUrl =
      normalizeOpenworkServerUrl(hostInfo?.baseUrl ?? "") ?? "";
  ...
  const createWorkerForSharedBundle = async (
    ...
    if (!hostUrl || !token) {
      throw new Error(
        "Share link detected. Configure an OpenWork worker host and token, then open the link again.",
      );
    }
  ...
  createEffect(() => {
    ...
    if (request.intent === "import_current") {
      ...
      if (!client || !workspaceId || !connected) {
        if (!sharedBundleNoticeShown()) {
          ...
          setError(
            "Share link detected. Connect to a writable OpenWork worker to import this bundle.",
          );
        }
        return;
      }
      ...
      if (!target.hostUrl.trim() || !target.token.trim()) {
        if (!sharedBundleNoticeShown()) {
          setSharedBundleNoticeShown(true);
          setError(
            "Share link detected. Configure an OpenWork host and token to create a new worker.",
          );
        }
        return;
      }
    ...
    void (async () => {
      try {
        ...
        const { payload, importedSkillsCount } =
          buildImportPayloadFromBundle(bundle);
        ...
        if (importedSkillsCount > 0) {
          console.log(
            `[openwork] imported ${importedSkillsCount} skills from share bundle`,
          );
        }
      } catch (error) {
        if (!cancelled) {
    ...
    const run = async () => {
      try {
        ...
        const activeMatch = response.activeId
          ...
          : null;
        const match = root
          ? items.find((item) => normalizeDirectoryPath(item.path) === root)
          : (activeMatch ?? items[0]);
  ...
  const openworkServerWorkspaceReady = createMemo(() =>
    Boolean(openworkServerWorkspaceId()),
  );
  ...
  const resolvedDevtoolsWorkspaceId = createMemo(
    () => devtoolsWorkspaceId() ?? openworkServerWorkspaceId(),
  );
  ...
  const [sharedBundleImportBusy, setSharedBundleImportBusy] =
    createSignal(false);
  const [sharedBundleNoticeShown, setSharedBundleNoticeShown] =
    createSignal(false);
  ...
  const editRemoteWorkspaceDefaults = createMemo(() => {
    ...
    return {
      ...
      openworkToken:
        workspace.openworkToken ?? openworkServerSettings().token ?? "",
  ...
  const testOpenworkServerConnection = async (next: OpenworkServerSettings) => {
    ...
    const result = await checkOpenworkServer(
      derived,
      next.token,
      openworkServerAuth().hostToken,
    );
    setOpenworkServerStatus(result.status);
    setOpenworkServerCapabilities(result.capabilities);
    setOpenworkServerCheckedAt(Date.now());
  ...
  const resolveOpenworkScheduler = () => {
    ...
    const client = openworkServerClient();
    const workspaceId = openworkServerWorkspaceId();
    if (openworkServerStatus() !== "connected" || !client || !workspaceId)
      return null;
    return { client, workspaceId };
  };

  const scheduledJobsSource = createMemo<"local" | "remote">(() => {
    return workspaceStore.activeWorkspaceDisplay().workspaceType === "remote"
      ? "remote"
      : "local";
  });
  ...
  const scheduledJobsSourceReady = createMemo(() => {
    ...
    const workspaceId = openworkServerWorkspaceId();
    return (
      openworkServerStatus() === "connected" && Boolean(client && workspaceId)
    );
  });

  const schedulerPluginInstalled = createMemo(() =>
    isPluginInstalledByName("opencode-scheduler"),
  );
  ...
  const refreshScheduledJobs = async (options?: { force?: boolean }) => {
    ...
    if (scheduledJobsSource() === "remote") {
      ...
      try {
        const response = await scheduler.client.listScheduledJobs(
  ...
  const deleteScheduledJob = async (name: string) => {
    if (scheduledJobsSource() === "remote") {
      ...
      const response = await scheduler.client.deleteScheduledJob(
        scheduler.workspaceId,
        name,
      );
      setScheduledJobs((current) =>
        current.filter((entry) => entry.slug !== response.job.slug),
      );
      return;
    }
    ...
    setScheduledJobs((current) =>
      current.filter((entry) => entry.slug !== job.slug),
    );
    return;
  };
  ...
  const loadInstructions = async () => {
    ...
    if (!client || !workspaceId || openworkServerStatus() !== "connected") {
      ...
      setInstructionsLastModified(null);
      return;
    }
    try {
      const result = await client.getInstructions(workspaceId);
      setInstructionsContent(result.content ?? "");
      setInstructionsLastModified(result.lastModified ?? null);
    } catch (e) {
  ...
  const handleSaveInstructions = async (
    ...
    const workspaceId = openworkServerWorkspaceId();
    if (!client || !workspaceId || openworkServerStatus() !== "connected") {
      return { ok: false, message: "Server not connected." };
    }
    try {
      await client.saveInstructions(workspaceId, content);
  ...
  // --- Commands ---
  const refreshCommands = async (options?: { force?: boolean }) => {
    const client = openworkServerClient();
    const workspaceId = openworkServerWorkspaceId();
    if (!client || !workspaceId || openworkServerStatus() !== "connected") {
      setCommands([]);
      setCommandsStatus("Server not connected.");
      return;
    }
    setCommandsStatus(null);
    try {
      const result = await client.listCommands(workspaceId);
      setCommands(Array.isArray(result.items) ? result.items : []);
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Failed to load commands";
      setCommands([]);
      setCommandsStatus(msg);
    }
  };

  const handleSaveCommand = async (input: {
    name: string;
    description?: string;
    template: string;
    agent?: string;
    ...
    const client = openworkServerClient();
    const workspaceId = openworkServerWorkspaceId();
    if (!client || !workspaceId) throw new Error("Server not connected.");
    await client.upsertCommand(workspaceId, input);
    await refreshCommands({ force: true });
  };
  ...
  const refreshSoulData = async (options?: { force?: boolean }) => {
    ...
    try {
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Failed to load soul heartbeats.";
        setActiveSoulHeartbeats([]);
      ...
      const message =
        error instanceof Error ? error.message : "Failed to load soul status.";
      setSoulStatusByWorkspaceId({});
      setActiveSoulHeartbeats([]);
      setSoulHeartbeatsBusy(false);
      setSoulError(message);
  ...
  const modelOptions = createMemo<ModelOption[]>(() => {
    ...
    for (const provider of sortedProviders) {
      ...
      for (const model of models) {
        ...
        if (model.reasoning)
          footerBits.push(t("settings.model_reasoning", currentLocale()));
  ...
  async function connectNotion() {
    if (workspaceStore.activeWorkspaceDisplay().workspaceType !== "local") {
      setNotionError(
        "Notion connections are only available for local workspaces.",
      );
      return;
    }
    ...
    try {
      if (canUseOpenworkServer) {
        ...
        const mcp =
          typeof nextConfig.mcp === "object" && nextConfig.mcp
        ...
        if (!result.ok) {
          throw new Error(
            result.stderr || result.stdout || "Failed to update opencode.json",
          );
        }
      }
      ...
      try {
        ...
        window.localStorage.setItem(
          "openwork.notionStatusDetail",
          t("mcp.connecting", currentLocale()),
        );
      ...
      setNotionError(
        e instanceof Error ? e.message : "Failed to connect Notion.",
      );
    } finally {
      setNotionBusy(false);
    }
  ...
  async function refreshMcpServers() {
    ...
    if (isRemoteWorkspace) {
      ...
      try {
        ...
        if (activeClient && projectDir) {
          try {
            const status = unwrap(
              await activeClient.mcp.status({ directory: projectDir }),
            );
    ...
    try {
      ...
      if (!config.exists || !config.content) {
        ...
        setMcpStatuses({});
        setMcpStatus(
          "No opencode.json found yet. Create one by connecting an MCP.",
        );
        return;
      }
  ...
  async function connectMcp(entry: (typeof MCP_QUICK_CONNECT)[number]) {
    ...
    if (!activeClient) {
      ...
      if (openworkBaseUrl && auth.token) {
        ...
        activeClient = createClient(opencodeUrl, undefined, {
          token: auth.token,
          mode: "openwork",
        });
    ...
    if (!resolvedProjectDir) {
      try {
        ...
        const discoveredRaw = normalizeDirectoryQueryPath(
          pathInfo.directory ?? "",
        );
        const discovered = discoveredRaw.replace(
          /^\/private\/tmp(?=\/|$)/,
          "/tmp",
        );
        if (discovered) {
    ...
    try {
      ...
      setMcpStatus(
        e instanceof Error
          ? e.message
          : t("mcp.connect_failed", currentLocale()),
      );
  ...
  async function logoutMcpAuth(name: string) {
    ...
    try {
      if (canUseOpenworkServer && openworkClient && openworkWorkspaceId) {
        ...
        try {
          await activeClient.mcp.disconnect({
            directory: resolvedProjectDir,
            name: safeName,
          });
        } catch {
        ...
        await activeClient.mcp.auth.remove({
          directory: resolvedProjectDir,
          name: safeName,
        });
      }
      ...
      try {
        const status = unwrap(
          await activeClient.mcp.status({ directory: resolvedProjectDir }),
        );
        setMcpStatuses(status as McpStatusMap);
      } catch {
        // ignore
      ...
      await refreshMcpServers();
      setMcpStatus(
        t("mcp.logout_success", currentLocale()).replace("{server}", safeName),
      );
    } catch (e) {
      setMcpStatus(
        e instanceof Error
          ? e.message
          : t("mcp.logout_failed", currentLocale()),
      );
    }
  }
  ...
  async function removeMcp(name: string) {
    try {
      ...
      setMcpStatus(
        e instanceof Error
          ? e.message
          : t("mcp.remove_failed", currentLocale()),
      );
    }
  }
  ...
  async function createSessionAndOpen() {
    ...
    try {
      ...
      const message =
        e instanceof Error
          ? e.message
          : t("app.unknown_error", currentLocale());
      setError(addOpencodeCacheHint(message));
      return undefined;
    } finally {
  ...
  onMount(async () => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (
          ...
          if (
            storedEngineSource === "custom" &&
            !(storedEngineCustomBinPath ?? "").trim()
          ) {
        ...
        if (
          storedEngineRuntime === "direct" ||
          storedEngineRuntime === "openwork-orchestrator"
        ) {
        ...
        const storedHideTitlebar = window.localStorage.getItem(
          HIDE_TITLEBAR_PREF_KEY,
        );
        ...
        const storedAutoCompactContext = window.localStorage.getItem(
          AUTO_COMPACT_CONTEXT_PREF_KEY,
        );
        ...
        if (
          storedUpdateAutoDownload === "0" ||
          storedUpdateAutoDownload === "1"
        ) {
        ...
        const storedNotionStatus = window.localStorage.getItem(
          "openwork.notionStatus",
        );
        if (
        ...
        const storedNotionDetail = window.localStorage.getItem(
          "openwork.notionStatusDetail",
        );
        ...
        const storedNotionSkillInstalled = window.localStorage.getItem(
          "openwork.notionSkillInstalled",
        );
    ...
    if (!isTauriRuntime()) {
      ...
      if (currentUrl) {
        ...
        const remoteStripped =
          stripRemoteConnectQuery(currentUrl) ?? currentUrl;
        const bundleStripped =
          stripSharedBundleQuery(remoteStripped) ?? remoteStripped;
  ...
  createEffect(() => {
    ...
    const raw = window.localStorage.getItem(
      sessionModelOverridesKey(workspaceId),
    );
    ...
    const writeConfig = async () => {
      try {
        if (canUseOpenworkServer) {
          ...
          const currentModel =
            typeof config.opencode?.model === "string"
              ? parseModelRef(config.opencode.model)
              : null;
  ...
  createEffect(() => {
    ...
    // Window decorations are only available in desktop environments (Tauri).
  });
  ...
  createEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        AUTO_COMPACT_CONTEXT_PREF_KEY,
        JSON.stringify(autoCompactContext()),
      );
    } catch {
      // ignore
    }
  ...
  const headerStatus = createMemo(() => {
    if (!client() || !headerConnectedVersion())
      return t("status.disconnected", currentLocale());
    const bits = [
      `${t("status.connected", currentLocale())} · ${headerConnectedVersion()}`,
    ];
  ...
  const workspaceSwitchWorkspace = createMemo(() => {
    ...
    if (switchingId) {
      return (
        workspaceStore.workspaces().find((ws) => ws.id === switchingId) ??
  ...
  const [workspaceSwitchDelayElapsed, setWorkspaceSwitchDelayElapsed] =
    createSignal(false);
  createEffect(() => {
    if (typeof window === "undefined") return;
    const switchingId = workspaceStore.connectingWorkspaceId();
    ...
    const timer = window.setTimeout(
      () => setWorkspaceSwitchDelayElapsed(true),
      250,
    );
  ...
  const workspaceSwitchOpen = createMemo(() => {
    ...
    if (workspaceStore.connectingWorkspaceId())
      return workspaceSwitchDelayElapsed();
    if (!busy() || !busyLabel()) return false;
    const label = busyLabel();
    return (
      label === "status.starting_engine" || label === "status.restarting_engine"
    );
  });

  const workspaceSwitchStatusKey = createMemo(() => {
    const label = busyLabel();
    if (label === "status.connecting")
      return "workspace.switching_status_connecting";
    if (
      label === "status.starting_engine" ||
    ...
    if (label === "status.loading_session")
      return "workspace.switching_status_loading";
    if (workspaceStore.connectingWorkspaceId())
      return "workspace.switching_status_loading";
  ...
  const dashboardProps = () => {
    ...
    return {
      ...
      soulError: soulError(),
      refreshSoulData: (options?: { force?: boolean }) =>
        refreshSoulData(options).catch(() => undefined),
      runSoulPrompt,
      activeWorkspaceRoot: workspaceStore.activeWorkspaceRoot().trim(),
      refreshSkills: (options?: { force?: boolean }) =>
        refreshSkills(options).catch(() => undefined),
      refreshHubSkills: (options?: { force?: boolean }) =>
        refreshHubSkills(options).catch(() => undefined),
      ...
      instructionsLastModified: instructionsLastModified(),
      loadInstructions,
      saveInstructions: handleSaveInstructions,
      // Commands
      commands: commands(),
      commandsStatus: commandsStatus(),
      refreshCommands: (options?: { force?: boolean }) => {
        refreshCommands(options).catch(() => undefined);
      },
    ...
    <>
      ...
      <CreateWorkspaceModal
        ...
        workerDisabledReason={(() => {
          if (!isTauriRuntime())
            return t("app.error.tauri_required", currentLocale());
          ...
          return (
            message || t("dashboard.sandbox_get_ready_desc", currentLocale())
          );
        })()}
        workerCtaLabel={t(
          "dashboard.sandbox_get_ready_action",
          currentLocale(),
        )}
        workerCtaDescription={t(
            ...
            try {
              ...
              if (ok) {
                ...
                setEditRemoteWorkspaceError(
                  error() || "Connection failed. Check the URL and token.",
                );
                setError(null);
              }
            } catch (e) {
