...
export default function SkillsView(props: SkillsViewProps) {
  ...
  const [uninstallTarget, setUninstallTarget] = createSignal<SkillCard | null>(
    null,
  );
  ...
  const [installLinkError, setInstallLinkError] = createSignal<string | null>(
    null,
  );
  const [installLinkBundle, setInstallLinkBundle] =
    createSignal<SkillBundleV1 | null>(null);

  const [selectedSkill, setSelectedSkill] = createSignal<SkillCard | null>(
    null,
  );
  ...
  const [installingSkillCreator, setInstallingSkillCreator] =
    createSignal(false);
  const [installingHubSkill, setInstallingHubSkill] = createSignal<
    string | null
  >(null);
  ...
  const installSkillCreator = async () => {
    ...
    try {
      ...
      setToast(
        e instanceof Error ? e.message : translate("skills.install_failed"),
      );
    } finally {
  ...
  const isValidKebabCase = (value: string) =>
  ...
  const submitCreateForm = async () => {
    ...
    if (!isValidKebabCase(name)) {
      setCreateError("Name must be kebab-case (e.g. my-skill-name).");
      return;
    }
    if (installedNames().has(name)) {
      setCreateError(`A skill named "${name}" already exists.`);
      return;
    }
    if (!body) {
      setCreateError("Skill content is required.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch bundle (${response.status})${suffix}`,
          );
        }
        const json = (await response.json()) as Record<string, unknown>;
        const schemaVersion =
          typeof json.schemaVersion === "number" ? json.schemaVersion : null;
  ...
  const installFromPreview = async (mode: "overwrite" | "keep-both") => {
    ...
    try {
      ...
      const finalName = shouldRename
        ? resolveUniqueSkillName(desiredName, taken)
        : desiredName;
      const content = shouldRename
  ...
  const recommendedDisabledReason = (id: string) => {
    if (id === "skill-creator") {
      ...
      if (props.busy || installingSkillCreator())
        return translate("skills.installing_skill_creator");
      if (!props.canInstallSkillCreator) {
  ...
  const openSkill = async (skill: SkillCard) => {
    ...
    try {
      ...
      setSelectedError(
        e instanceof Error ? e.message : "Failed to load skill.",
      );
    } finally {
  ...
  const saveSelectedSkill = async () => {
    ...
    try {
      ...
      setSelectedError(
        e instanceof Error ? e.message : "Failed to save skill.",
      );
    }
  };
  ...
  const workspaceLabel = createMemo(
    () => props.workspaceName.trim() || "Worker",
  );
  ...
  const isOpenworkInjectedSkill = (skill: SkillCard) => {
    ...
    return (
      OPENWORK_DEFAULT_SKILL_NAMES.has(normalizedName) ||
  ...
  return (
    <section class="space-y-8">
      ...
      <div class="space-y-2">
        <h2 class="text-3xl font-bold text-dls-text">
      ...
      <div class="rounded-2xl border border-dls-border bg-dls-surface px-5 py-5 shadow-[0_8px_26px_rgba(17,24,39,0.05)]">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div class="min-w-0 space-y-1">
            ...
            <div class="text-xl font-semibold text-dls-text truncate">
              {workspaceLabel()}
            </div>
            <p class="text-sm text-dls-secondary">
              Skills are the core abilities of this worker. Add from Hub or
              create new ones directly in chat.
            </p>
          </div>
          <div class="flex items-center gap-2">
            <button
              ...
              disabled={props.busy}
              class={`inline-flex items-center gap-1.5 rounded-lg border border-dls-border px-4 py-2 text-xs font-medium transition-colors ${
                props.busy
                  ? "bg-dls-hover text-dls-secondary"
                  : "bg-dls-surface text-dls-text hover:bg-dls-active"
              }`}
            >
              <FileText size={14} />
              Create from form
                ...
                <div class="bg-dls-surface border border-dls-border rounded-xl p-4 flex items-start justify-between gap-4 group hover:border-dls-border hover:bg-dls-hover transition-all text-left">
                  <div class="flex gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                      ...
                      >
                        <p class="text-xs text-dls-secondary line-clamp-2">
                          {skill.description}
                        </p>
                      </Show>
          ...
          <For each={recommendedSkills()}>
            {(item) => (
              <div
                ...
                title={
                  item.disabled
                    ? (recommendedDisabledReason(item.id) ?? item.title)
                    : item.title
                }
              >
                <div class="flex gap-4 min-w-0">
                  ...
                  <div class="min-w-0">
                    ...
                    <Show
                      when={
                        item.id === "skill-creator" &&
                        !props.canInstallSkillCreator &&
                        !skillCreatorInstalled()
                      }
                    >
                      <div class="mt-1 text-[11px] text-dls-secondary">
                        {props.accessHint ??
                          translate("skills.host_only_error")}
                      </div>
                    </Show>
                  </div>
                </div>
                ...
                >
                  <Show
                    when={
                      item.id === "skill-creator" && installingSkillCreator()
                    }
          ...
          <div class="w-full max-w-4xl rounded-2xl border border-dls-border bg-dls-surface shadow-2xl overflow-hidden">
            ...
            <div class="p-5">
              ...
              <Show
                ...
                fallback={
                  <div class="text-xs text-dls-secondary">Loading…</div>
                }
              >
                <textarea
          ...
          <div class="w-full max-w-2xl rounded-2xl border border-dls-border bg-dls-surface shadow-2xl overflow-hidden">
            <div class="px-5 py-4 border-b border-dls-border flex items-center justify-between gap-3">
              <div>
                ...
                <div class="text-xs text-dls-secondary">
                  Define a skill with structured fields
                </div>
              </div>
              <div class="flex items-center gap-2">
                <button
            ...
            <div class="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div class="space-y-1.5">
                ...
                <input
                  ...
                  class="w-full bg-dls-hover border border-dls-border rounded-lg px-3 py-2 text-xs font-mono text-dls-text focus:outline-none focus:ring-2 focus:ring-[rgba(var(--dls-accent-rgb)/0.25)]"
                  spellcheck={false}
                />
                <div class="text-[10px] text-dls-secondary">
                  Must be kebab-case (lowercase, hyphens only). Example:{" "}
                  <code>audit-helper</code>
                </div>
              </div>
              <div class="space-y-1.5">
                <label class="text-xs font-semibold uppercase tracking-widest text-dls-secondary">
              ...
              <div class="space-y-1.5">
                <label class="text-xs font-semibold uppercase tracking-widest text-dls-secondary">
                  Skill content <span class="text-red-500">*</span>
                </label>
                <textarea
                  value={createBody()}
                  ...
                  class="w-full min-h-[280px] rounded-xl border border-dls-border bg-dls-hover px-4 py-3 text-xs font-mono text-dls-text focus:outline-none focus:ring-2 focus:ring-[rgba(var(--dls-accent-rgb)/0.25)]"
                  spellcheck={false}
                />
                <div class="text-[10px] text-dls-secondary">
                  Write the skill instructions in markdown. The frontmatter
                  (name, description) will be auto-generated.
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="text-lg font-semibold text-dls-text">
                    {translate("skills.uninstall_title")}
                  </h3>
                  <p class="text-sm text-dls-secondary mt-1">
                    {translate("skills.uninstall_warning").replace(
                      "{name}",
          ...
          <div class="bg-dls-surface border border-dls-border w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div>
                ...
                <p class="text-sm text-dls-secondary mt-1">
                  Publish a public link. Anyone with the URL can install this
                  skill.
                </p>
              </div>

              <div class="rounded-xl border border-dls-border bg-dls-hover px-4 py-3 text-xs text-dls-secondary">
                <div class="font-semibold text-dls-text">
                  {shareTarget()?.name}
                </div>
                <div class="mt-1 font-mono break-all">
                  Publisher: {DEFAULT_OPENWORK_PUBLISHER_BASE_URL}
                </div>
              </div>
          ...
          <div class="bg-dls-surface border border-dls-border w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              ...
              <div class="flex justify-end gap-2">
                ...
                >
                  {installLinkBusy() && !installLinkBundle()
                    ? "Loading…"
                    : "Preview"}
                </Button>
