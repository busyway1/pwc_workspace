...
export type DashboardViewProps = {
  ...
  instructionsLastModified: string | null;
...
export default function DashboardView(props: DashboardViewProps) {
  ...
  const [lastRefreshedTab, setLastRefreshedTab] = createSignal<string | null>(
    null,
  );
  ...
  createEffect(() => {
    ...
    const doRefresh = async () => {
      try {
        ...
        if (currentTab === "instructions" && !cancelled) {
          await props.loadInstructions();
        }
        if (currentTab === "commands" && !cancelled) {
          await props.refreshCommands();
        }
      } catch {
  ...
  const soulNavIconClass = () =>
    soulModeEnabled() ? "soul-nav-icon-active" : "";

  const navItem = (t: DashboardTab, label: string, icon: any) => {
    const active = () =>
      props.tab === t || (t === "mcp" && props.tab === "plugins");
    return (
      <button
        type="button"
    ...
    <div class="flex h-screen w-full bg-dls-surface text-dls-text font-sans overflow-hidden">
      ...
      <main class="flex-1 flex flex-col overflow-hidden bg-dls-surface">
            ...
            <Switch>
              ...
              <Match when={props.tab === "commands"}>
                <CommandsView
                  ...
                  commandsStatus={props.commandsStatus}
                  refreshCommands={props.refreshCommands}
                  saveCommand={props.saveCommand}
                  deleteCommand={props.deleteCommand}
                />
              </Match>

              <Match when={props.tab === "instructions"}>
                <InstructionsView
                  busy={props.busy}
                  content={props.instructionsContent}
                  lastModified={props.instructionsLastModified}
                  loadInstructions={props.loadInstructions}
                  saveInstructions={props.saveInstructions}
                />
              </Match>
              ...
              <Match when={props.tab === "config" && props.developerMode}>
                <ConfigView
                  ...
                  resetOpenworkServerSettings={
                    props.resetOpenworkServerSettings
                  }
                  testOpenworkServerConnection={
                    props.testOpenworkServerConnection
                  }
                  canReloadWorkspace={props.canReloadWorkspace}
                  reloadWorkspaceEngine={props.reloadWorkspaceEngine}
                  reloadBusy={props.reloadBusy}
              ...
              <Match when={props.tab === "settings"}>
                <SettingsView
                  ...
                  cleanupOpenworkDockerContainers={
                    props.cleanupOpenworkDockerContainers
                  }
        ...
        <nav class="md:hidden border-t border-dls-border bg-dls-surface">
          <div
            class={`mx-auto max-w-5xl px-4 py-3 grid gap-2 ${props.developerMode ? "grid-cols-6" : "grid-cols-5"}`}
          >
            <button
      ...
      <aside class="w-56 hidden md:flex flex-col bg-dls-sidebar border-l border-dls-border p-4">
        <div class="space-y-1 pt-2">
          ...
          {navItem("instructions", "Instructions", <FileText size={18} />)}
